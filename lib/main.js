...
function listWorldInGUI(worldName, exists) {
    (exists) ? $('#worlds-list-container').append('<a href="#"><li class="world-item world-item-exists">' + worldName + '</li></a>')
             : $('#worlds-list-container').append('<a href="#"><li class="world-item world-item-deleted">' + worldName + '</li></a>');
}
...
function showNoMinecraftError() {
    $('#worlds-container').hide();
    $('.new-snapshot-btn').hide();
    $('#snapshots-container').css('width', '100%');
    $('.titlebar').css('padding-bottom', '10px');
}
...
$(document).ready(function() {
    ...
    if (!fs.existsSync(minecraftSavesDir)) {
        showNoMinecraftError();
        return false;
    }
